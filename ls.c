...
static void my_write(const char *s)
{
    size_t len = 0;
    while (s[len]) len++;
    /* write() écrit directement dans stdout, sans printf. */
    write(STDOUT_FILENO, s, len);
}

static void my_write_nl(void)
{
    /* write() permet aussi d'écrire un seul caractère. */
    write(STDOUT_FILENO, "\n", 1);
}
...
{
    /* open() ouvre le répertoire et donne un descripteur de fichier. */
    ...
    /* getdents64() lit plusieurs entrées du répertoire dans un buffer. */
    while ((nread = syscall(SYS_getdents64, fd, buf, BUF_SIZE)) > 0) {
        ...
        for (long pos = 0; pos < nread; ) {
            ...
            if (!(d->d_name[0] == '.' &&
                ...
                /* stat() obtient les droits, le type, la taille, etc. */
                char fullpath[4096];
